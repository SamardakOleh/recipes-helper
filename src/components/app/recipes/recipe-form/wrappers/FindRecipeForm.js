...
class FindRecipeForm extends Component {
  ...
  find() {
    let filteredRecipes = this.props.recipes.filter(recipe => {
      ...
      if (this.state.formRecipe.products)
        selectedProductNames = this.state.formRecipe.products.map(product => {
          return product.name.toLowerCase();
        });

      for (let selectedProduct of selectedProductNames)
        if (!recipeProductNames.includes(selectedProduct)) return acceptable;
      if (
        recipe.name.includes(this.state.formRecipe.name) &&
